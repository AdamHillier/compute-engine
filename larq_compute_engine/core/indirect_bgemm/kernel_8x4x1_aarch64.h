...
inline void AccumulationLoop_Depth2OrMore(
    ...
    const std::int32_t input_depth_offset, int32x4_t weights[2],
  ...
  TBitpacked* a_ptr_0 = indirection_buffer[0] + input_depth_offset + 1;
  TBitpacked* a_ptr_1 = indirection_buffer[1] + input_depth_offset + 1;
  TBitpacked* a_ptr_2 = indirection_buffer[2] + input_depth_offset + 1;
  TBitpacked* a_ptr_3 = indirection_buffer[3] + input_depth_offset + 1;
...
inline void OutputTransformAndLoadNextAndStore(
    ...
    int32x4_t weights[2], int32x4_t activations[4],
  ...
  asm("ldp x0, x1, [%[indirection_buffer]]\n\t"
      ...
      "add x0, x0, %[input_depth_offset], lsl #2\n"
...
void RunKernel(const std::int32_t block_num_pixels,
  ...
  if (conv_kernel_size < 1 || input_depth_per_group < 1 ||
  ...
  do {
    ...
    if (Depth2OrMore) {
      AccumulationLoop_Depth2OrMore(
          conv_kernel_size, input_depth_per_group, input_depth_offset, weights,
          activations, accumulators, weights_ptr, indirection_buffer);
    } else {
