...
inline void AccumulationLoop_Depth2OrMore(
  ...
  TBitpacked* a_ptr_0 = indirection_buffer[0] + input_depth_offset + 2;
  TBitpacked* a_ptr_1 = indirection_buffer[1] + input_depth_offset + 2;
  TBitpacked* a_ptr_2 = indirection_buffer[2] + input_depth_offset + 2;
  TBitpacked* a_ptr_3 = indirection_buffer[3] + input_depth_offset + 2;
...
void RunKernel(const std::int32_t block_num_pixels,
  ...
  if (conv_kernel_size < 1 || input_depth_per_group < 2 ||
  ...
  do {
    ...
    if (Depth2OrMore) {
      AccumulationLoop_Depth2OrMore(
          conv_kernel_size, input_depth_per_group / 2, input_depth_offset,
