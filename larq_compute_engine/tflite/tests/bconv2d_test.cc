...
struct TestParam {
  ...
  static std::string TestNameSuffix(
    ...
    return absl::Substitute(
        "Op$0_I$1_K$2_P$3_PV$4_S$5_D$6_$7_Act$8_T$9", param.kernel_name,
        param_input_oss.str(), param_filter_oss.str(), GetPaddingName(padding),
        pad_values, param_strides_oss.str(), param_dilation_oss.str(),
        param_bitpacked_input, getActivationString(param.activation),
        param.num_threads);
  }
...
class BaseBConv2DOpModel : public SingleOpModel {
 public:
  BaseBConv2DOpModel(
      ...
      int pad_values = 0, int dilation_width_factor = 1,
      int dilation_height_factor = 1, bool read_bitpacked_input = false,
      enum ActivationFunctionType activation = ActivationFunctionType_NONE,
      int num_threads = -1) {
    input_ = AddInput(input);
    filter_ = AddInput(filter);
...
TEST_P(BConv2DOpTest, SimpleTest) {
  ...
  BConv2DOpModel m_lce(
      ...
      read_bitpacked_input, activation, num_threads);
  ...
  EXPECT_DEATH(
      {
        BConv2DOpModel m_lce(compute_engine::tflite::Register_BCONV_2D64,
                             ...
                             {TensorType_FLOAT32, {}}, 1, 1, Padding_SAME, 0, 1,
                             1, false, ActivationFunctionType_RELU, 1);
      },
